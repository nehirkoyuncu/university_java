...
// R1
		...
		IO.println("Rector of " + poli.getName() + " : " + poli.getRector());
		
// R2
		int s1 = poli.enroll("Mario","Rossi");
		int s2 = poli.enroll("Giuseppe","Verdi");
		
		IO.println("Enrolled students " + s1 + ", " + s2); // 10000, 10001
		IO.println("s1 = " + poli.student(s1)); // 10000 Mario Rossi

// R3		
		int macro = poli.activate("Macro Economics", "Paul Krugman");
		int oop = poli.activate("Object Oriented Programming", "James Gosling");
		
		IO.println("Activated courses " + macro + " and " + oop); // 10 and 11
...
// R4
		...
		IO.println(poli.listAttendees(macro));
		// 10000 Mario Rossi
		// 10001 Giuseppe Verdi
		
		IO.println(poli.studyPlan(s2));
...
// R5
		...
		IO.println(poli.studentAvg(s2)); // 29.0
		
		IO.println(poli.courseAvg(macro)); // 28.5
        
// R6
		IO.println("Best students:\n" + poli.topThreeStudents());
	}
}
